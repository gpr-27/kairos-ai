...
function wsUrl(): string {
  return `${getSocketUrl()}/ws/playground`;
}
    ...
    return (
      <div className="flex h-full flex-col overflow-hidden bg-[#0d0d10]">
              ...
              {lines.map((line) => (
                <div
                  ...
                  className={cn(
                    ...
                    line.kind === 'info' && 'italic text-zinc-500',
        ...
        {running && (
          <form
            onSubmit={sendStdin}
            className="border-border/60 flex shrink-0 items-center gap-2 border-t bg-[#111115] px-4 py-2"
          >
            <span className="shrink-0 select-none font-mono text-sm text-emerald-400">›</span>
            <input
              ref={inputRef}
              type="text"
              value={inputVal}
              ...
              className="min-w-0 flex-1 bg-transparent font-mono text-sm text-zinc-100 outline-none placeholder:text-zinc-600 disabled:opacity-40"
              autoComplete="off"
              autoCorrect="off"
              spellCheck={false}
            />
            <kbd className="hidden text-[10px] text-zinc-600 sm:inline">Enter ↵</kbd>
          </form>
        )}
      </div>
    );

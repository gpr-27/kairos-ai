...
export default function ProfilePage(): JSX.Element {
    ...
    return (
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
        <header>
          ...
          <p className="text-muted-foreground mt-2">Using Kairos as a guest.</p>
        </header>
        <div className="mt-8">
          <GuestUpgradeCard
            title="Create an account to unlock your profile"
            description="Profiles and cross-device sync need a free account. Your guest chats and submissions move over automatically when you sign up."
          />
        </div>
      </div>
    );
  ...
  const STATS = [
    ...
    { label: 'Streak', value: `${streak}d`, icon: Flame },
    {
      ...
      sub: `${totalSolved} / ${SAMPLE_PROBLEMS.filter((p) => p.track === 'dsa').length} problems`,
      icon: Target,
    },
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            ...
            <div>
              ...
              {recentSolves.length === 0 ? (
                ...
                <ul className="divide-border/50 [&::-webkit-scrollbar-track]:bg-muted/30 [&::-webkit-scrollbar-thumb]:bg-border hover:[&::-webkit-scrollbar-thumb]:bg-muted-foreground/50 max-h-72 divide-y overflow-y-auto pr-1 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar]:w-1.5">

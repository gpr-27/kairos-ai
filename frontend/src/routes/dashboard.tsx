    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <Reveal immediate>
        <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-primary mb-1 text-sm font-medium">{greeting},</p>
            ...
            <p className="text-muted-foreground mt-2">
              {totalSolved === 0
                ? "You haven't solved any problems yet — let's change that."
                : `You've solved ${totalSolved} problem${totalSolved !== 1 ? 's' : ''}. Keep the momentum going.`}
            </p>
          </div>
      ...
      {/* Quick stats */}
      <Stagger className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-5">
        {QUICK_STATS.map((stat) => (
          <motion.div key={stat.label} variants={fadeUp}>
            <Card className="card-hover border-border/60 h-full">
              <CardContent className="flex items-center gap-2.5 p-3.5 sm:gap-3 sm:p-5">
                <div
                  className={`bg-gradient-to-br ${stat.bg} ring-border/40 shrink-0 rounded-xl p-2 ring-1 sm:p-2.5 ${stat.accent}`}
                >
                  <stat.icon className="h-5 w-5" />
                </div>
                <div className="min-w-0">
                  <p className="text-muted-foreground truncate text-xs uppercase tracking-wider">
                    {stat.label}
                  </p>
                  <p className="text-xl font-bold leading-tight tabular-nums sm:text-2xl">
                    {stat.value}
                  </p>
                </div>
              </CardContent>
              ...
              {(
                ...
                <div key={label}>
                  ...
                  <div className="bg-muted h-2 w-full overflow-hidden rounded-full">
                    <motion.div
                      initial={{ width: 0 }}
                      whileInView={{ width: total ? `${(solved / total) * 100}%` : '0%' }}
                      viewport={{ once: true }}
        ...
        <section className="mt-10">
          ...
          <Stagger className="mt-4 grid gap-4 md:grid-cols-3">
            {recommended.map((problem) => (
              <motion.div key={problem.slug} variants={fadeUp}>
                <Link
                  to={`/problem/${problem.slug}`}
                  className="card-hover border-border/60 bg-card/50 hover:border-primary/40 group block h-full rounded-xl border p-5 backdrop-blur-sm"
                >
                  <div className="flex items-start justify-between gap-2">
                    <h3 className="group-hover:text-primary text-base font-semibold leading-tight transition-colors">
                      {problem.title}
                    </h3>
                    <Badge
                      variant={DIFFICULTY_VARIANTS[problem.difficulty]}

...
const PROBLEMS: SeedProblem[] = [
  // ─── EASY ───────────────────────────────────────────────────────────────
  {
    ...
    hints: ['Brute force is O(n²). Can we do better?', 'A hash map gives O(1) lookup: "have I seen target − nums[i] before?"'],
  },

  {
    ...
    starterCode: { python: PYTHON, cpp: CPP, java: JAVA },
    hints: ['When you see an open bracket, what should you remember?', 'A stack (LIFO) naturally matches the most recent unmatched open bracket.'],
  },

  {
    ...
    description: 'Given prices array, choose one day to buy and a later day to sell. Return maximum profit (0 if impossible).',
    ...
    starterCode: { python: PYTHON, cpp: CPP, java: JAVA },
    hints: ['Track the minimum price seen so far.', 'At each day: best profit if selling today = price − min_so_far.'],
  },
...

  {
    ...
    difficulty: 'easy',
    topics: ['arrays', 'hashing'],
    description: 'Return true if any value appears at least twice, false if all elements are distinct.',
...

  {
    ...
    starterCode: { python: PYTHON, cpp: CPP, java: JAVA },
    hints: ['At each position: extend or start fresh? `current = max(nums[i], current + nums[i])`.'],
  },

  {
    ...
    description: 'Return array where each element is the product of all other elements. O(n), no division.',
...

  {
    ...
    description: 'Return the fewest coins needed to make `amount`, or −1 if impossible. Unlimited supply of each coin.',
    ...
    starterCode: { python: PYTHON, cpp: CPP, java: JAVA },
    hints: ['dp[i] = min coins to make amount i. dp[0] = 0.', 'For each i and coin c: dp[i] = min(dp[i], dp[i−c] + 1).'],
  },

  {
    ...
    description: "Count islands in an m×n grid of '1' (land) and '0' (water). Adjacent land cells (horizontal/vertical) form one island.",
...

  {
    ...
    difficulty: 'medium',
    topics: ['linked_lists'],
    description: 'Reverse a singly linked list. Input: first line = n, second line = n space-separated values.',
  ...
  // ─── HARD ─────────────────────────────────────────────────────────────
  {
    ...
    starterCode: { python: PYTHON, cpp: CPP, java: JAVA },
    hints: ['Water at position i = min(maxLeft, maxRight) − height[i].', 'Two-pointer: the smaller-boundary side determines water level.'],
  },

  {
    ...
    description: 'Return true if `word` exists in the grid using sequentially adjacent cells (no reuse). Input: first line = m n, next m lines = board rows, last line = word.',
  ...
  // ─── MORE EASY ──────────────────────────────────────────────────────────
  {
    ...
    difficulty: 'easy',
    topics: ['math'],
    description: 'Given an integer x, return true if x is a palindrome, false otherwise. Negative numbers are never palindromes.',
    constraints: ['-2³¹ ≤ x ≤ 2³¹ − 1'],
    ...
    starterCode: { python: PYTHON, cpp: CPP, java: JAVA },
    hints: ['Negative numbers are never palindromes.', 'Try reversing only the second half of the number and compare.'],
  },
...

  {
    ...
    description: 'Given n integers, return the element that appears more than ⌊n/2⌋ times. Input: line 1 = n, line 2 = n integers.',
...

  {
    ...
    description: 'Move all 0s to the end while keeping relative order of non-zero elements. Input: line 1 = n, line 2 = n integers.',
    constraints: ['1 ≤ n ≤ 10⁴'],
...

  {
    ...
    difficulty: 'easy',
    topics: ['arrays', 'math'],
    description: 'Given n distinct numbers in [0,n], return the missing one. Input: line 1 = n, line 2 = n integers.',
    constraints: ['1 ≤ n ≤ 10⁴'],
...

  {
    ...
    difficulty: 'easy',
    topics: ['linked_lists'],
    description: 'Merge two sorted integer sequences. Each given as a single space-separated line (empty line for empty list).',
    ...
    starterCode: { python: PYTHON, cpp: CPP, java: JAVA },
    hints: ['Two pointers, always pick the smaller head. Append the remainder when one list runs out.'],
  },
  ...
  // ─── MORE MEDIUM ────────────────────────────────────────────────────────
  {
    ...
    description: 'Return all unique triplets [a,b,c] from the array that sum to 0. Print each triplet sorted ascending, one per line, groups sorted lexicographically. Input: line 1 = n, line 2 = n integers.',
    ...
    starterCode: { python: PYTHON, cpp: CPP, java: JAVA },
    hints: ['Sort first. Fix nums[i], two-pointer the rest.', 'Skip duplicate values at every pointer position.'],
  },

  {
    ...
    description: 'Given n heights, find the two lines that form a container holding the most water. Input: line 1 = n, line 2 = n heights.',
...

  {
    ...
    description: 'Given a string s, return the longest palindromic substring. If tied, any valid answer is accepted.',
...

  {
    ...
    description: 'Group the anagrams. Print each group on a separate line, words sorted alphabetically, groups sorted by first word. Input: line 1 = n, line 2 = n space-separated strings.',
    constraints: ['1 ≤ n ≤ 10⁴', '0 ≤ word.length ≤ 100'],
    examples: [{ input: 'n=6, strs=[eat,tea,tan,ate,nat,bat]', output: 'ate eat tea\nbat\nant nat tan' }],
    testCases: [
...

  {
    ...
    difficulty: 'medium',
    topics: ['arrays', 'greedy'],
    description: 'Each element is the max jump from that index. Return true if you can reach the last index. Input: line 1 = n, line 2 = n integers.',
...

  {
    ...
    difficulty: 'medium',
    topics: ['arrays', 'sorting'],
    description: 'Merge all overlapping intervals. Input: line 1 = n, lines 2..n+1 each have two integers "start end". Print each merged interval on its own line.',
    ...
    starterCode: { python: PYTHON, cpp: CPP, java: JAVA },
    hints: ['Sort by start. Merge when current.start ≤ prev.end (update prev.end = max of both ends).'],
  },

  {
    ...
    description: 'Rotate the array right by k steps. Input: line 1 = n, line 2 = n integers, line 3 = k.',
  ...
  // ─── MORE HARD ──────────────────────────────────────────────────────────
  {
    ...
    description: 'Return the minimum window substring of s that contains every character in t (with duplicates). Return "" if none. Input: line 1 = s, line 2 = t.',
...

  {
    ...
    description: 'Given n bar heights (width 1 each), return the area of the largest rectangle. Input: line 1 = n, line 2 = n heights.',

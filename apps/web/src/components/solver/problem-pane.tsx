...
import { ChevronDown, ChevronRight, Info, Lightbulb, MessageSquare, Terminal, TestTube2 } from 'lucide-react';
...
function JudgeExplainer() {
  ...
  return (
    <div className="border-border/60 bg-blue-500/5 rounded-lg border">
      <button
        className="flex w-full items-center gap-2.5 px-4 py-3 text-left"
        onClick={() => setOpen((o) => !o)}
      >
        <Info className="text-blue-400 h-4 w-4 shrink-0" />
        ...
        <div className="border-border/60 border-t px-4 pb-4 pt-3">
          <div className="space-y-3 text-sm">
            <p className="text-muted-foreground leading-relaxed">
              This judge works <span className="text-foreground font-semibold">exactly like Codeforces</span> or
              any competitive-programming platform:
            </p>
            <ol className="text-muted-foreground space-y-2 pl-4">
              <li className="list-decimal">
                <span className="text-foreground font-medium">Write a complete program</span> — you must include
                the{' '}
                <code className="bg-muted rounded px-1 py-0.5 font-mono text-xs">main()</code> function (or
                equivalent).
              </li>
              <li className="list-decimal">
                ...
                <code className="bg-muted rounded px-1 py-0.5 font-mono text-xs">Scanner</code>. Read in the{' '}
                <span className="text-foreground font-semibold">exact order</span> shown in the I/O Format
                below.
              </li>
              <li className="list-decimal">
                <span className="text-foreground font-medium">Print your answer to stdout</span> — use{' '}
                <code className="bg-muted rounded px-1 py-0.5 font-mono text-xs">cout</code> /{' '}
              ...
              <li className="list-decimal">
                Click <span className="text-foreground font-medium">Run</span> — the judge feeds each test
                case's stdin into your program and compares your output to the expected output.
              </li>
            </ol>
            <div className="bg-amber-500/10 border-amber-500/30 rounded border p-3">
              ...
              <p className="text-muted-foreground mt-1 text-xs">
                ...
                <code className="bg-muted rounded px-0.5 font-mono">target</code> is on line 3, read the full
                array first, then read{' '}
          ...
          <pre className="bg-muted/40 overflow-x-auto rounded p-2 font-mono text-xs leading-relaxed">
            {lines.map((line, i) => (
              <span key={i} className="flex">
                <span className="text-muted-foreground/40 mr-2.5 select-none w-3 text-right shrink-0">
                  {i + 1}
                </span>
                <span>{line || ' '}</span>
              </span>
  ...
  return (
    <div className="bg-card/30 flex h-full flex-col overflow-hidden">
      ...
      <Tabs defaultValue="description" className="flex min-h-0 flex-1 flex-col">
        <div className="border-border/60 border-b px-4 pt-2">
                  ...
                  <section>
                    ...
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                      <div>
                        ...
                        <pre className="bg-muted/50 border-border/60 mt-1 rounded-md border p-3 font-mono text-xs leading-relaxed whitespace-pre">
              ...
              <div className="space-y-3 px-5 py-4">
                ...
                {problem.hints.map((hint, i) => (
                  <details key={i} className="group">
                    <summary className="border-border/60 hover:bg-muted/40 flex cursor-pointer list-none items-center gap-2 rounded-md border px-4 py-3 text-sm font-medium select-none">

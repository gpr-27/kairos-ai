...
import { ChevronLeft, ChevronRight, Loader2, MessageSquarePlus, RefreshCw, ThumbsUp, Trash2 } from 'lucide-react';
...
function Avatar({ src, name }: { src: string; name: string }) {
  ...
  return (
    <div className="bg-primary/20 text-primary flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[10px] font-bold ring-1 ring-border/60">
      {initials}
    </div>
  );
}
...
function CommentCard({
  ...
  return (
    <div className="group border-border/60 rounded-lg border bg-background/40 p-3.5 transition-colors hover:bg-background/70">
      {/* Header */}
      ...
      <div className="mt-2.5 flex items-center gap-1">
        <button
          ...
          title={currentUserId ? (comment.upvotedByMe ? 'Remove upvote' : 'Upvote') : 'Sign in to upvote'}
        >
        ...
        <div className="space-y-2.5 px-3 py-3">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center gap-2 py-10 text-center">
              ...
              <p className="text-muted-foreground/70 text-xs">Be the first to share your approach!</p>
            </div>
          ) : (
            comments.map((c) => (
              <CommentCard

...
function TestCaseRow({ tc, defaultOpen }: { tc: TestCaseResult; defaultOpen: boolean }) {
  ...
  return (
    <li className="border-border/60 border-b last:border-0">
      ...
      {/* Detail panel */}
      {open && (
        <div className="border-border/40 bg-background/40 grid grid-cols-1 gap-0 border-t sm:grid-cols-3 sm:divide-x sm:divide-border/40">
          <div className="p-3">
      ...
      {open && tc.stderr && (
        <div className="border-border/40 bg-destructive/5 border-t px-4 pb-3 pt-2">
          ...
          <pre className="text-destructive/80 overflow-x-auto font-mono text-[11px] leading-relaxed whitespace-pre-wrap">
            {tc.stderr}
          </pre>
        </div>
      )}
    ...
    return (
      <div className="border-border/60 bg-card/30 flex h-full items-center gap-2 border-t px-4">
        ...
        <span className="text-muted-foreground text-xs">
          Click <span className="text-foreground font-medium">Run</span> to execute your code against all test cases.
        </span>
      </div>
    );
  }
      ...
      <ScrollArea className="min-h-0 flex-1">
        ...
        {compileOutput && (
          <div className="border-border/60 border-b p-4">
            ...
            <pre className="bg-destructive/10 text-destructive overflow-x-auto rounded-md p-3 font-mono text-xs leading-relaxed whitespace-pre-wrap">
              {compileOutput}
            </pre>
          </div>
        )}

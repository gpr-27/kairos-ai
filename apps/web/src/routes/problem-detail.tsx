...
export default function ProblemDetailPage(): JSX.Element {
  ...
  const effectiveLang = (problem?.starterCode[language] !== undefined ? language : firstAvailable) as Language;
  ...
  async function handleRun(): Promise<void> {
    ...
    try {
      ...
      const response = await fetch(`${getApiBaseUrl()}/api/v1/submissions/run`, {
        ...
        body: JSON.stringify({ problemId: problem.slug, language: effectiveLang, code: editorCode }),
      });
      ...
      toast.error(
        msg.includes('Run failed')
          ? 'Could not reach the backend. Is the API running on :4000?'
          : msg,
      );
    } finally {
      setRunning(false);
  ...
  return (
    <div className="flex h-[calc(100dvh-4rem)] flex-col">
      ...
      <PanelGroup direction="horizontal" className="flex-1 overflow-hidden" autoSaveId="kairos-solver-h">
        {/* ── Left: Problem pane ── */}
        <Panel
          ref={problemRef}
          defaultSize={28}

    ...
    <PanelResizeHandle
      className={cn(
        ...
        direction === 'horizontal'
          ? 'w-1.5 cursor-col-resize hover:bg-primary/10 active:bg-primary/20'
          : 'h-1.5 cursor-row-resize hover:bg-primary/10 active:bg-primary/20',
      )}
    >
      <div
        className={cn(
          'rounded-full bg-border/50 transition-all group-hover:bg-primary/40 group-active:bg-primary/70',
...
function MobileLayout({
  selectedLang, code, editorReady, containerRef, terminalRef, onCodeChange, onMount, onRunningChange,
}: {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      <div className="border-border/60 flex shrink-0 border-b bg-card/10">
...
export default function PlaygroundPage(): JSX.Element {
  ...
  useEffect(() => { monacoRef.current?.updateOptions({ fontSize, tabSize }); }, [fontSize, tabSize]);
    ...
    <div className="flex h-[calc(100dvh-4rem)] flex-col">
      ...
      <div className="border-border/60 bg-card/20 flex shrink-0 items-center gap-2 border-b px-3 py-1.5">
        ...
        <div className="ml-auto flex items-center gap-2">
          <Button variant="ghost" size="sm" onClick={handleReset} disabled={running} className="gap-1.5">
          ...
          >
            {running
              ...
              : <><Play className="h-3.5 w-3.5" />Run<kbd className="bg-primary/20 hidden rounded px-1 py-0.5 text-[9px] font-medium sm:inline">⌘↵</kbd></>
            }
          </Button>
        </div>
      </div>

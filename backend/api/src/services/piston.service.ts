...
async function loadRuntimes(): Promise<void> {
  ...
  try {
    const response = await fetch(`${config.piston.baseUrl}/runtimes`);
    if (!response.ok) {
...

  {
    ...
    if (!version) {
      if (hasLocalRunner) {
        logger.warn(
          { lang: args.language },
          'No Piston runtime found; using local fallback runner',
        );
        return executeCodeLocally(args);
      }
    ...
    try {
      const response = await fetch(`${config.piston.baseUrl}/execute`, {
        method: 'POST',

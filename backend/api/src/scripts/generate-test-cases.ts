...
async function generateWithGroq(
  ...
  const groqApiKey = config.llm.groqApiKey;
  if (!groqApiKey) {
...
async function validateTestCase(
  ...
  try {
    ...
    const validated =
      result.exitCode === 0 && !result.compileError && actualOutput === expectedOutput;
...
async function run(): Promise<void> {
  ...
  if (failed.length > 0) {
    ...
    for (const f of failed) {
      logger.warn(
        { input: f.input, expected: f.expectedOutput, got: f.actualOutput, error: f.error },
        '  ✗',
      );
    }
  }

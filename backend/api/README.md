...
Full request/response shapes live in [`backend/types/src/`](../../backend/types/src/).

## Adding an endpoint

1. **Type it** in `backend/types/src/<domain>.ts` (request + response, with Zod schema).
2. **Service** function in `backend/api/src/services/<domain>.service.ts`.
3. **Route** in `backend/api/src/routes/<domain>.routes.ts`:
   ```ts
   router.post(
     '/foo',
     requireUser,

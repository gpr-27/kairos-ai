...
class GroqProvider:
    ...
    def __init__(self, model: str | None = None) -> None:
        ...
        # A generous timeout keeps a hung upstream from stalling the SSE stream forever.
    ...
    def _model_chain(self, primary: str) -> list[str]:
        """Effective primary model first; fallback second (if different)."""
        fb = _fallback_model()
        models = [primary]
        if primary != fb:
            models.append(fb)
    ...
    async def stream(self, request: LLMRequest) -> AsyncIterator[str]:
        ...
        primary = request.model or self._model
        logger.debug(f"[groq] streaming model={primary} msgs={len(request.messages)}")

        last_exc: Exception | None = None

        for model in self._model_chain(primary):
            # Track how many tokens this attempt has emitted. If we've already
            # streamed part of an answer and then hit a rate-limit, restarting on
            # the fallback model would replay text the user has already seen, so we
            # stop gracefully instead — the caller finalizes the partial response.
            yielded = 0
            try:
                ...
                if yielded > 0:
                    logger.warning(
                        f"[groq] rate-limited on {model} after {yielded} tokens — "
                        "keeping partial response instead of restarting on fallback"
                    )
                    return
                if model == fb:
